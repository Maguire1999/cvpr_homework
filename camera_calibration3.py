...
# 获取标定板角点的位置  
objp = np.zeros((corners_size[0]*corners_size[1],3), np.float32)  

objp[:,:2] = np.mgrid[0:corners_size[0],0:corners_size[1]].T.reshape(-1,2)  # 将世界坐标系建在标定板上，所有点的Z坐标全部为0，所以只需要赋值x和y  
  
obj_points = []    # 存储3D点  
img_points = []    # 存储2D点  
# vscode images = glob.glob(".\\data\\cam_data_resized\\3cm\\*.jpg")
# charm images = glob.glob("..\\data\\cam_data_resized\\3cm\\*.jpg")
images = glob.glob(".\\data\\cam_data_resized\\3cm\\*.jpg")
...
for fname in images:  
    ...
    if ret:  
        ...
        cv2.drawChessboardCorners(img,corners_size, corners, ret)   
        # cv2.imshow('img', img)  
        # cv2.waitKey(0)  
